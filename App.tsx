...
import { Button, Modal, SOSButton, Input, Checkbox, Avatar, BadgeIcon, Tabs, Select } from './components/Shared';
import { 
  User as UserIcon, MapPin, Flame, HandHeart, Car, ChevronLeft, Smartphone, Mail, Lock, Bell, Navigation, Camera, ArrowRight, Calendar, Clock, Euro, Briefcase, PawPrint, MessageCircle, Siren, CheckCircle, XCircle, Send, Award, UserPlus, UserCheck, Users, MessageSquare, Trash2, AlertCircle, Compass, Eye, EyeOff, Luggage, Info, HeartHandshake
} from 'lucide-react';
...
const GlobalChatContent: React.FC<{ 
    ...
    // Mock Active Conversations List based on messages or static mock
    // In a real app, this would be derived from the user's active chats
...
// --- COMPOSANT DÉTAIL RICHE (UPDATED WITH GROUP CHAT) ---
const ActivityDetailContent: React.FC<{ 
    ...
    lang: Language
}> = ({ activity, onClose, onChat, onJoin, lang }) => {
    ...
    const handleGroupChat = () => {
        onChat({
            ...
            avatar: activity.image || activity.organizer.avatar, // Fallback
    ...
    // Category Translation Helper (Fallback to original string if not found)
    ...
    return (
        <div className="flex flex-col h-full -mx-6 -my-4 sm:mx-0 sm:my-0">
            ...
            <div className="p-6 bg-white flex-1 overflow-y-auto">
                ...
                {/* Organizer Card (FIXED: NOW SHOWS INTENTION) */}
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl mb-6 border border-gray-100 relative">
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex gap-1 mt-1">
                            {activity.organizer.badges.slice(0,3).map(b => <span key={b.id} title={b.name} className="text-xs">{b.icon}</span>)}
                        </div>
                    </div>
                    {/* Direct Chat button (Only if not me) */}
                    {!isOrganizer && (
                    ...
                    {isRide && (
                        <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 space-y-3">
                            ...
                            <div className="border-t border-blue-200 pt-3 flex gap-4 text-sm text-blue-800">
                                ...
                                {activity.animals && <div className="flex items-center gap-1"><PawPrint size={14}/> Animaux OK</div>}
                            </div>
                        </div>
                    )}

                    {isHelp && activity.priceMode === 'TROC' && (
                        <div className="bg-orange-50 p-4 rounded-2xl border border-orange-100">
                            <h4 className="font-bold text-orange-800 mb-1 flex items-center gap-2"><HeartHandshake size={18}/> Échange Proposé (Troc)</h4>
                            <p className="text-orange-900 italic">"{activity.exchangeDetails}"</p>
                        </div>
                    )}
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2">
                             <div className="flex -space-x-2">
                                 {/* Show Fake participants + ME if joined */}
                         ...
                         {isParticipating ? (
                            <div className="flex gap-3 w-full">
                             ...
                             <>
                                ...
                                >
                                    {isRide ? (activity.subtype === 'OFFER' ? t.status.book : t.status.propose) : 
                                    isHelp ? (activity.subtype === 'OFFER' ? t.status.contact : t.status.propose) : 
                                    t.status.join}
                                </Button>
                             </>
                         )}
...
// ... Auth Components (Login, Signup, CGU, Profile, Intention) kept same ...
...
const App: React.FC = () => {
  ...
  const handleJoinActivity = (activity: Activity) => {
      // 1. Update the state for immediate feedback
      const updatedActivity = { ...activity, myStatus: 'PARTICIPANT' as const, participants: activity.participants + 1 };
      ...
      setNotifications([
          {id: `n_${Date.now()}`, type: 'INFO', title: 'Inscription confirmée', message: `Vous participez à : ${activity.title}`, date: 'À l\'instant', read: false},
          ...notifications
      ]);
  };
      ...
      <div className="flex-1 relative overflow-hidden bg-gray-100 flex flex-col">
        ...
        {(currentTab === AppTab.MOMENTS || currentTab === AppTab.ENTRAIDE || currentTab === AppTab.TRAJETS) && 
            <ServicesModule 
                ...
                user={user} // Pass dynamic user here
            />
        }
      </div>
