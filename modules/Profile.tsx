...
// --- SECURITY COMPONENT (Updated with Friend Selection) ---
const SecurityContent: React.FC<any> = ({ friends, sosContacts, setSosContacts, sosPhone, setSosPhone, privacyMode, setPrivacyMode, isPremium, onCancelPremium }) => {
    ...
    const toggleContact = (friendName: string) => {
        if (sosContacts.includes(friendName)) {
            setSosContacts(sosContacts.filter((c:string) => c !== friendName));
        } else {
            if (sosContacts.length < 3) {
                setSosContacts([...sosContacts, friendName]);
            }
        }
    };
        ...
        <div className="space-y-6 pb-6">
            ...
            <div className="bg-white p-4 rounded-xl space-y-4 border border-gray-100 shadow-sm">
                ...
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <Toggle checked={ghostMode} onChange={ghostMode} />
                </div>
            ...
            <div className="bg-white p-4 rounded-xl space-y-4 border border-gray-100 shadow-sm">
                ...
                <div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-2 max-h-40 overflow-y-auto">
                        {friends.map((f: any) => (
                            <div key={f.id} className="flex items-center justify-between p-2 border-b border-gray-200 last:border-0">
                                <div className="flex items-center gap-2">
                                    <Avatar src={f.avatar} size="xs" />
                                    <span className="text-sm font-medium">{f.name}</span>
                                </div>
                                <Checkbox 
                                    checked={sosContacts.includes(f.name)} 
                                    onChange={() => toggleContact(f.name)} 
                                    label=""
                                />
                            </div>
                        ))}
